...
List endpoints return a `data` array alongside a `page_info` object with `has_next_page`, `has_prev_page`, and `next_page_url` / `previous_page_url`. To page forward, read the `cursor` query parameter from `next_page_url` and pass it into the next `list` call. You can also set `limit` to control page size.
...
for (;;) {
    ...
    if (!page.page_info.has_next_page || !page.page_info.next_page_url) break;

    // The next-page URL carries the cursor as a query parameter.
    const cursor = new URL(page.page_info.next_page_url).searchParams.get('cursor');
    page = await client.catalog.items.list({ cursor: cursor ?? undefined });
}
```
