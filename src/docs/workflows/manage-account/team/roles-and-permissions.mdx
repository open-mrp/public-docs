...
Augno ships a small number of global roles. They are shared across Augno rather than owned by your account, so they show up in your roles list but cannot be renamed, edited, or deleted:
...
A number of domains are declared but are not currently checked by any endpoint — `shifts`, `supplies`, `receiving`, `department_picks`, `error_logs`, `scanning_error_logs`, `inventory_change_logs`, `product_groups`, `product_variations`, `production_step_transformations`, and `shipping_cases` among them. Granting or withholding them has no effect today.
