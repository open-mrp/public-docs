...
- **Downgrading to the free plan turns agents off immediately.** New runs are refused and scheduled runs are skipped from that moment, not at period close.
...
**How it is metered.** Every model call an agent makes is routed through a metered LLM gateway that reports token usage to Stripe against your account's Stripe customer as the call happens — segmented by model and by token type (input versus output). Augno keeps its own per-run token totals for analytics, but the billable quantities are the ones Stripe holds. Sandbox agent usage is billed to the production account that owns the sandbox, not to the sandbox.

**What the dashboard number means.** The **Estimated Spend This Month** figure on the billing page is not a local guess from an internal price list. Augno reconstructs it the way Stripe will bill it: for each model and token type, metered quantity multiplied by your plan's rate-card unit price, summed. Those rate-card prices already include your plan's markup, so the figure and your eventual invoice are computed from the same inputs. It is called "estimated" for two narrow reasons: Stripe's usage analytics have a short freshness lag, and Augno caches the computed value briefly. Expect it to trail live usage by a few minutes, not to differ in method.
