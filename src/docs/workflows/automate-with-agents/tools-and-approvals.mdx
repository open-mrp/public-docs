...
**Built-in tools** are OpenMRP's own capabilities, attached directly to the agent definition. There are five:

| Tool              | Group     | What it does                                                                                                  |
| ----------------- | --------- | ------------------------------------------------------------------------------------------------------------- |
| `create_artifact` | General   | Produces an artifact — a report, document, or data export — with a name, MIME type, and content               |
| `read_doc`        | Knowledge | Reads a page of OpenMRP's documentation, discovering what is available from the docs site's `llms.txt` index    |
...
`send_email` and `draft_reply` look similar and are not. `draft_reply` stages a message and stops; a person edits and approves it, and OpenMRP then delivers it over whichever channel the case uses — email if the case is bridged to an inbox, an in-app portal message otherwise. `send_email` actually sends.

**Endpoint tools** are OpenMRP's own API operations exposed to the agent as tools — over 200 of them, covering the same list/create/update operations your integrations use. They are granted separately from built-ins, through an explicit allowlist on the agent.
...
Injecting the full tool catalog into every model turn would drown the agent in context and make it worse at its job, so OpenMRP does not do that. Instead, an agent that has been granted at least one endpoint tool gets a single meta-tool, `search_api_tools`.
...
3. OpenMRP writes a **plain-language summary of what the agent is asking to do**, generated from the run's own context so raw IDs appear as the names the agent already looked up. If a summary cannot be produced, the run falls back to naming the tools it is blocked on.
