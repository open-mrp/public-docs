...
The fastest way to use an agent is to talk to it. <BetaTag /> An agent can be a participant in any conversation — a thread with your team, your own notes, or a customer case — and it answers in the thread like anyone else. Nothing to schedule, nothing to configure: mention it and it goes to work.
...
> #### Preview
>
> Adding and removing an agent participant is an **internal preview** operation. Both endpoints are
> documented below with their real paths and payloads, but neither is in the public OpenAPI spec or the
> public SDKs, and either may change without notice. Read the
>
...
Two details to plan around. **`trigger_keywords` is replaced wholesale, not merged** — send the complete list every time, because omitting it on a re-add clears the keywords the agent had. And **`agent_config_id` is not checked against your agent catalog when the participant is created**, so a wrong id is accepted here and only surfaces later as an agent that never answers.
...
Those three actions are available on the API, and unlike the rest of the agent surface they are **public**:
...
> #### Preview
>
> These three endpoints are in **public preview**. They are in the public spec and the SDKs, but their
> shapes may still change while the messaging surface stabilizes.
...
Only the first approval of a draft can send it. The promote is a compare-and-set on the draft's status, so a concurrent double-approve cannot put the message in front of the customer twice — a second attempt is rejected as a conflict rather than sending again.
