...
| ---------- | ----------------------------------------- | --------------------------------------------------------------------- |
| **System** | Provided by OpenMRP, shared across accounts | No — a system definition is read-only and cannot be edited or deleted |
...
The **system prompt** is the agent's standing brief: its role, its constraints, what "done" looks like. It is sent to the model on every turn of every run, alongside the platform guidance OpenMRP adds automatically (how to discover API tools, how to expand related records, and so on).
...
You never pick a model. You pick a **tier**, and OpenMRP resolves it to a model for you.
