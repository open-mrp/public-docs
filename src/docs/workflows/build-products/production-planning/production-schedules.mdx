...
Augno does not schedule the whole factory. It schedules the one department that sets the pace — the constraint — and derives every other department's work from it by walking the <InternalLink pathKey="buildProducts.bom.productionFlows" text="production flow" />.
