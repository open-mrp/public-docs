...
Each job shows the SKU, the <InternalLink pathKey="buildProducts.bom.productionSteps" text="production step" /> and department that will do it, the quantity implied, and how far downstream of the constraint it sits — `+1w · step 1` is work that waits only on the constraint campaign feeding it, `step 3` waits on two intermediate steps. That is deliberately a timing signal rather than a readiness signal: nothing writes shop-floor progress onto derived work, so a chip claiming "ready to start" would be inventing state.
