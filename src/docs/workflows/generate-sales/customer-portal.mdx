...
**Preview**: the portal-domain and portal-host endpoints behind this feature are in preview. Some of them are not published in the public OpenAPI spec or the SDKs, and all of them may change without notice.
...
**Preview**: the rate-shopping endpoint behind these options, and the `exemption_type` values it returns, are in preview. The endpoint is not published in the public OpenAPI spec or the SDKs, and it may change without notice.
...
**Preview**: the messaging endpoints behind the portal's support flow are in preview. Several of them, including contact support and support availability, are not published in the public OpenAPI spec or the SDKs, and all of them may change without notice.
