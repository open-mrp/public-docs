...
/**
 * Extracts simple crumbs array from breadcrumbs
 */
function extractCrumbs(
    breadcrumbs: Array<{ label: string; pathKey?: string }> | undefined,
    currentName: string,
): string[] {
    if (!breadcrumbs || breadcrumbs.length === 0) {
        return [currentName];
    }
    // Get labels from breadcrumbs, excluding 'Home', and add current name
    const crumbs = breadcrumbs.filter((b) => b.label !== 'Home').map((b) => b.label);
...
async function indexDocs() {
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const crumbs = extractCrumbs(frontmatter.breadcrumbs, name);
